...
public class SpecialSleepEventCommand {
    ...
    private static int runResetAll(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        context.getSource().sendFeedback(() -> Text.translatable("feedback.nemuelch.world_data.reset"), true);
        return 1;
    }
    ...
    private static int runResetSpecific(CommandContext<ServerCommandSource> context, int commandInputId) throws CommandSyntaxException {
        ...
        for (int i = 0; i < persistentWorldData.usedSleepEventEntries.size(); i++) {
            ...
            if (entry.sleepEventIndex() == commandInputId) {
                ...
                context.getSource().sendFeedback(() -> Text.translatable("feedback.nemuelch.world_data.reset_specific", commandInputId), true);
                return 1;
            }
        }
        context.getSource().sendError(Text.translatable("feedback.nemuelch.world_data.reset_specific_error", commandInputId));
        return 0;
    }
    ...
    private static int runListEntries(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        for (var entry : persistentWorldData.usedSleepEventEntries) {
            MutableText playerText = Text.literal(entry.playerName()).styled(style -> style.withColor(Formatting.AQUA));
            Text entryInformation = playerText.append(Text.literal(" activated entry Index: " + entry.sleepEventIndex()));
        ...
        if (!persistentWorldData.usedSleepEventEntries.isEmpty()) {
        ...
        context.getSource().sendFeedback(() -> Text.literal(leftOverEntries).styled(style -> style.withColor(Formatting.GREEN)), true);
        return 1;
    }
}
