...
import net.shirojr.nemuelch.entity.ai.ChaseAllButSummonerGoal;
import net.shirojr.nemuelch.entity.ai.OnionIgniteGoal;
...
public class OnionEntity extends HostileEntity implements IAnimatable {
    ...
    private int explosionRadius = NeMuelchConfigInit.CONFIG.onion.getEntityData().getExplosionRadius();
    private final float effectRadius = NeMuelchConfigInit.CONFIG.onion.getEntityData().getEffectRadius();
    ...
    public OnionEntity(EntityType<OnionEntity> livingEntityEntityType, World world) {
        this(world, null);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.onion.walk", ILoopType.EDefaultLoopTypes.LOOP));
            return PlayState.CONTINUE;
        }

        if (this.dataTracker.get(IGNITED)) {

            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.onion.explode", ILoopType.EDefaultLoopTypes.PLAY_ONCE));
            return PlayState.CONTINUE;
        }

        event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.onion.idle", ILoopType.EDefaultLoopTypes.LOOP));
        return PlayState.CONTINUE;
    }
    ...
    public static DefaultAttributeContainer.Builder setAttributes() {
        return HostileEntity.createHostileAttributes()
                .add(EntityAttributes.GENERIC_MAX_HEALTH, NeMuelchConfigInit.CONFIG.onion.getEntityData().getMaxHealth())
                .add(EntityAttributes.GENERIC_MOVEMENT_SPEED, NeMuelchConfigInit.CONFIG.onion.getEntityData().getMovSpeed())
                .add(EntityAttributes.GENERIC_FOLLOW_RANGE, NeMuelchConfigInit.CONFIG.onion.getEntityData().getFollowRange());
    }
    ...
    private void explode() {
        if (!this.world.isClient()) {
            ...
            if (!this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING) ||
                    !NeMuelchConfigInit.CONFIG.onion.getEntityData().getEnvironmentalDamage()) {
    ...
    protected SoundEvent getAmbientSound() {
        ...
        if (chance < 1) {
            return NeMuelchSounds.ENTITY_ONION_SQUEEL_ONE;
        }
        if (chance < 2) {
            return NeMuelchSounds.ENTITY_ONION_SQUEEL_TWO;
